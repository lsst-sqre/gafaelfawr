...
async def test_login_no_destination(tmp_path: Path) -> None:
    app = await create_fastapi_test_app(tmp_path)
...
async def test_claim_names(tmp_path: Path, responses: aioresponses) -> None:
    """Uses an alternate settings environment with non-default claims."""
    app = await create_fastapi_test_app(
        tmp_path, username_claim="username", uid_claim="numeric-uid"
    )
    setup = SetupFastAPITest(tmp_path, responses)
    assert setup.config.github
    setup.set_github_token_response("some-code", "some-github-token")
...
async def test_bad_redirect(tmp_path: Path, responses: aioresponses) -> None:
    app = await create_fastapi_test_app(tmp_path)
    setup = SetupFastAPITest(tmp_path, responses)
    setup.set_github_token_response("some-code", "some-github-token")
    userinfo = GitHubUserInfo(
