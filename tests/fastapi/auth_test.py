...
async def test_invalid(tmp_path: Path) -> None:
    app = await create_fastapi_test_app(tmp_path)
...
async def test_invalid_auth(tmp_path: Path) -> None:
    app = await create_fastapi_test_app(tmp_path)
    ...
    async with create_test_client(app) as client:
        ...
        token = create_test_token(config(), exp=int(exp), scope="exec:admin")
        r = await client.get(
            "/auth",
            params={"scope": "exec:admin"},
...
async def test_access_denied(tmp_path: Path) -> None:
    app = await create_fastapi_test_app(tmp_path)
...
async def test_auth_forbidden(tmp_path: Path) -> None:
    app = await create_fastapi_test_app(tmp_path)
...
async def test_satisfy_all(tmp_path: Path) -> None:
    app = await create_fastapi_test_app(tmp_path)
    token = create_test_token(config(), scope="exec:test")
...
async def test_success_any(tmp_path: Path) -> None:
    ...
    """
    app = await create_fastapi_test_app(tmp_path)
    token = create_test_token(config(), groups=["test"], scope="exec:test")
...
async def test_basic(tmp_path: Path) -> None:
    app = await create_fastapi_test_app(tmp_path)
    token = create_test_token(config(), groups=["test"], scope="exec:admin")
...
async def test_basic_failure(tmp_path: Path) -> None:
    app = await create_fastapi_test_app(tmp_path)
    ...
    async with create_test_client(app) as client:
        ...
        for basic in (b"foo:foo", b"x-oauth-basic:foo", b"foo:x-oauth-basic"):
            ...
            assert r.status_code == 401
            authenticate = parse_www_authenticate(
...
async def test_handle(tmp_path: Path) -> None:
...
async def test_reissue_internal(tmp_path: Path) -> None:
    """Test requesting token reissuance to an internal audience."""
    app = await create_fastapi_test_app(tmp_path)
    token = create_test_token(config(), groups=["admin"], scope="exec:admin")
    ...
    async with create_test_client(app) as client:
        r = await client.get(
            ...
            params={
                "scope": "exec:admin",
                "audience": config().issuer.aud_internal,
            },
            headers={"Authorization": f"Bearer {token.encoded}"},
        )
    ...
    decoded_token = jwt.decode(
        new_encoded_token,
        config().issuer.keypair.public_key_as_pem(),
    ...
    assert decoded_token == {
        ...
        "aud": config().issuer.aud_internal,
        "email": token.email,
        "exp": ANY,
        "iat": ANY,
        ...
        "iss": config().issuer.iss,
        "jti": ANY,
        "scope": " ".join(sorted(config().issuer.group_mapping["admin"])),
...
async def test_ajax_unauthorized(tmp_path: Path) -> None:
